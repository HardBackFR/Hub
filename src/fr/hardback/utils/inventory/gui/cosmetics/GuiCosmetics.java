...
public class GuiCosmetics extends AbstractGui {
    ...
    public void onInventoryClick(InventoryClickEvent event){
        ...
        if(event.getView().getTitle().equalsIgnoreCase("Menu des cosmétiques")){
            event.setCancelled(true);

            if(itemStack.getItemMeta().getDisplayName().equalsIgnoreCase(ChatColor.RED + "Firework")){
                player.closeInventory();
                if(isVip(player)){
                    player.getInventory().addItem(new ItemBuilder(Material.FIREWORK_ROCKET).setName(ChatColor.RED + "Firework").toItemStack());
                }else{
                ...
                if(isVip(player)){
                    if(petsManager.petIsSpawn()){
                        ...
                        player.playSound(player.getLocation(), Sound.ENTITY_PLAYER_LEVELUP, 3.0F, 0.533F);
